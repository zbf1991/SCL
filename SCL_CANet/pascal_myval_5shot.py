...
import argparse
from pascal_dataset_mask_val_5shot_orisize import Dataset as Dataset_val
import os
import torch
from SCL_5shot import Res_Deeplab
...
parser.add_argument('-gpu',type=str,help='gpu id to use',default='0,1')
...
for eva_iter in range(options.iter_time):
    ...
    for i_iter, batch in enumerate(valloader):
        ...
        query_rgb, query_mask, support_rgb_list, support_mask_list, history_mask, sample_class, index = batch
        iou = []
        for list_i in range(len(support_rgb_list)):
            iou_query_rgb = support_rgb_list[list_i]
            iou_query_mask = support_mask_list[list_i]
            iou_query_mask_all = torch.cat([iou_query_mask, iou_query_mask, iou_query_mask, iou_query_mask, iou_query_mask], dim=0)
            iou_histoty = torch.zeros([iou_query_mask.shape[0],2,iou_query_mask.shape[2],iou_query_mask.shape[3]])
            iou_query_pred = model(iou_query_rgb, support_rgb_list, support_mask_list, iou_histoty, False)[3]
            iou_query_pred = torch.stack(iou_query_pred, dim=0).squeeze(dim=1)
            iou_query_pred = F.interpolate(iou_query_pred,size=iou_query_mask.size()[-2:], mode='bilinear', align_corners=False)
            iou_query_pred_label = torch.argmax(iou_query_pred, dim=1).float()
            iou.append(np.array(get_iou_v1(iou_query_mask_all.cpu(), iou_query_pred_label.float().cpu())[2]))
        iou_mean = np.mean(np.array(iou),axis=0)
        ...
        iou_mean = torch.from_numpy(iou_mean).cuda().float()
        for scale in [0.7, 1, 1.3]:
            query_ = F.interpolate(query_rgb, scale_factor=scale, mode='bilinear', align_corners=False)
            scale_pred_list = model(query_, support_rgb_list, support_mask_list, history_mask, True)[3]
            scale_pred_tensor = torch.stack(scale_pred_list, dim=0).squeeze(dim=1)
            scale_pred = iou_mean.view(5,1,1,1)*scale_pred_tensor

            scale_pred = torch.mean(scale_pred, dim=0, keepdim=True)

            pred_softmax += F.interpolate(scale_pred, size=query_rgb.size()[-2:], mode='bilinear', align_corners=False)

        pred_softmax = F.softmax(pred_softmax, dim=1)

        history_pred_softmax = pred_softmax.clone().data.cpu()
        # update history mask
        for j in range(query_rgb.shape[0]):
            sub_index = index[j]
