...
import tqdm
import argparse
from pascal_dataset_mask_train import Dataset as Dataset_train
from pascal_dataset_mask_val import Dataset as Dataset_val
import os
import torch
...
parser.add_argument('-lr',type=float,help='learning rate',default=0.00025)
parser.add_argument('-prob',type=float,help='dropout rate of history mask',default=0.9)
parser.add_argument('-bs',type=int,help='batchsize',default=4)
parser.add_argument('-bs_val',type=int,help='batchsize for val',default=16)
...
for epoch in range(0,num_epoch):
    ...
    for i_iter, batch in enumerate(tqdm_gen):
        ...
        query_mask_cnn = query_mask.cuda(0)
        ...
        pred, supp_pred, new_supp_pred =model(query_rgb, support_rgb, support_mask,history_mask)
        ...
        supp_pred = nn.functional.interpolate(supp_pred, size=input_size, mode='bilinear', align_corners=True)  # upsample
        new_supp_pred = nn.functional.interpolate(new_supp_pred, size=input_size, mode='bilinear',
                                              align_corners=True)  # upsample

        support_label = torch.squeeze(support_mask,dim=1)
        qry_loss = loss_calc_v1(pred, query_mask, 0)
        supp_loss = loss_calc_v1(supp_pred, support_label, 0) + loss_calc_v1(new_supp_pred, support_label, 0)
        loss = qry_loss + supp_loss
    ...
    with torch.no_grad():
        ...
        for eva_iter in range(options.iter_time):
            ...
            for i_iter, batch in enumerate(valloader):
                ...
                pred = model(query_rgb, support_rgb, support_mask,history_mask)[0]
                pred_softmax = F.softmax(pred, dim=1).data.cpu()

                # update history mask
                query_rgb_size = query_rgb.shape[-2:]
                for j in range(support_mask.shape[0]):
                    sub_index = index[j]
                    valset.history_mask_list[sub_index] = pred_softmax[j]
                    # pred = nn.functional.interpolate(pred, size=query_rgb_size, mode='bilinear',
                    #                                  align_corners=True)
