...
from model.PFENet_SCL_5shot import PFENet
...
save_img_path = './exp/coco/split3_resnet101/vis(5shot)'

def get_parser():
    parser = argparse.ArgumentParser(description='PyTorch Semantic Segmentation')
    parser.add_argument('--config', type=str, default='config/coco/coco_split0_resnet101.yaml', help='config file')
...
def validate(val_loader, model, criterion):
    ...
    for e in range(20):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            if sum(intersection.float() / union.float()) / 2 >= 0.9:
                q_pred = output[:, :ori_label.size(1), :ori_label.size(2)]
                q_pred = torch.squeeze(q_pred).cpu().numpy()
                cv2.imwrite(os.path.join(save_img_path, str(args.split) + str(i) + str(e) + 'query_pred5.png'), 255 * q_pred.astype(np.uint8))
