...
class PFENet(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), y=None):
        ...
        #   Query Feature
        query_feat, query_feat_3, query_feat_4 = generate_query_feat(self, x)

        #   Support Feature     
        supp_feat_list = []
        out_all_list = []
        iou_all = torch.zeros([self.shot])
        ...
        for i in range(self.shot):
            ...
            with torch.no_grad():
                ...
                supp_feat_0_list.append(supp_feat_0)
                supp_feat_1_list.append(supp_feat_1)
                supp_feat_2_list.append(supp_feat_2)
            ...
            supp_feat_map_list.append(supp_feat_map)
            ...
            for idx, tmp_bin in enumerate(self.pyramid_bins):
                ...
                merge_supp_feat_bin = F.interpolate(merge_supp_feat_bin,
                                                    size=(supp_feat_map.size(2), supp_feat_map.size(3)),
                                                    mode='bilinear', align_corners=True)
                supp_pyramid_feat_list.append(merge_supp_feat_bin)
                supp_out_list.append(inner_supp_out_bin)

            supp_pyramid_feat_list_list.append(supp_pyramid_feat_list)

            init_supp_feat_map = torch.cat(supp_pyramid_feat_list, 1)
            init_supp_feat_map = self.res1(init_supp_feat_map)
            init_supp_feat_map = self.res2(init_supp_feat_map) + init_supp_feat_map
            supp_out = self.cls(init_supp_feat_map)
            supp_pred_mask = torch.argmax(supp_out, dim=1, keepdim=True)
            supp_pred_mask_list.append(supp_pred_mask)

        for i in range(self.shot):
            final_supp_list_for_corr = []
            final_supp_list_for_corr.append(final_supp_list[i])
            mask_list_for_corr = []
            mask_list_for_corr.append(mask_list[i])
            # supp_feat_2 = supp_feat_2_list[i]
            supp_feat_3 = supp_feat_3_list[i]
            mask = F.interpolate(mask_list[i], size=(supp_feat_3.size(2), supp_feat_3.size(3)), mode='bilinear', align_corners=True)

            corr_query_mask_list = []
            cosine_eps = 1e-7
            corr_query_mask = util.compute_prior_mask(final_supp_list_for_corr, mask_list_for_corr, query_feat_4, query_feat_3,
                                                      corr_query_mask_list, query_feat, cosine_eps)

            supp_feat1, supp_feat2 = SA_Weighted_GAP(supp_feat_map_list[i], mask, supp_pred_mask_list[i])

            # new_supp_out = compute_supp_pred(self, supp_feat_map, supp_feat1, supp_feat2, supp_pyramid_feat_list)

            for j in range(self.shot):
                # sq_x = s_x[:,j,:,:,:]
                # sq_feat, sq_feat_3, sq_feat_4 = generate_query_feat(self, sq_x)
                # corr_sq_mask_list = []
                # corr_sq_mask = util.compute_prior_mask(final_supp_list, mask_list, sq_feat_4, sq_feat_3,
                #                                           corr_sq_mask_list, sq_feat, cosine_eps)

                sq_out = compute_supp_pred(self, supp_feat_map_list[j], supp_feat1, supp_feat2, supp_pyramid_feat_list_list[j])
                # sq_out = compute_query_pred(self, sq_feat, supp_feat1, supp_feat2, corr_sq_mask)
                sq_out = F.interpolate(sq_out, size=(h, w), mode='bilinear', align_corners=True)
                intersection, union, new_target = util.intersectionAndUnionGPU(sq_out.max(1)[1], s_y[:, j, :, :], K=2)
                iou = torch.mean((intersection.float() / union.float()))
                iou_all[i] += iou

            out = compute_query_pred(self,query_feat,supp_feat1, supp_feat2, corr_query_mask)

            #   Output Part
            if self.zoom_factor != 1:
                out = F.interpolate(out, size=(h, w), mode='bilinear', align_corners=True)
                out_all_list.append(out)

        iou_all = iou_all/self.shot
        out_f = 0
        for i, out in enumerate(out_all_list):
            out_f += iou_all[i]*out
...
def compute_supp_pred(self,supp_feat_map, supp_feat1, supp_feat2, supp_pyramid_feat_list):
    new_supp_out_list = []
...
def compute_query_pred(self, query_feat, supp_feat1, supp_feat2, corr_query_mask):
    out_list = []
    pyramid_feat_list = []
    ...
    for idx, tmp_bin in enumerate(self.pyramid_bins):
        ...
        merge_feat_bin = F.interpolate(merge_feat_bin, size=(query_feat.size(2), query_feat.size(3)), mode='bilinear',
                                       align_corners=True)
        pyramid_feat_list.append(merge_feat_bin)
        out_list.append(inner_out_bin)

    query_feat_post = torch.cat(pyramid_feat_list, 1)
    query_feat_post = self.res1(query_feat_post)
    query_feat_post = self.res2(query_feat_post) + query_feat_post
    out = self.cls(query_feat_post)

    return out

def generate_query_feat(self,x):
    with torch.no_grad():
    ...
    return query_feat, query_feat_3, query_feat_4